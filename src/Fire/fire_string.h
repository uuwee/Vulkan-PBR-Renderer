...
STR_API size_t STR_IntToStrEx(char* buffer, uint64_t data, bool is_signed, int radix); // NOT null-terminated, the size of the string is returned.
STR_API size_t STR_FloatToStrEx(char* buffer, double value, int min_decimals); // NOT null-terminated, the size of the string is returned.
...
STR_API STR_View STR_FloatToStr(void* allocator, double value, int min_decimals) {
	char buffer[100];
	size_t size = STR_FloatToStrEx(buffer, value, min_decimals);
...
STR_API STR_View STR_IntToStrEx(void* allocator, uint64_t data, bool is_signed, int radix) {
	char buffer[100];
	size_t size = STR_IntToStrEx(buffer, data, is_signed, radix);
	STR_View value_str = { buffer, size };
	return STR_Clone(allocator, value_str);
}

STR_API size_t STR_IntToStrEx(char* buffer, uint64_t data, bool is_signed, int radix) {
...
STR_API size_t STR_FloatToStrEx(char* buffer, double value, int min_decimals) {
	size_t offset = 0;
	if (isnan(value)) {
	...
	offset += STR_IntToStrEx(buffer + offset, integralPart, false, 10);
	...
	if (exponent < 0) {
		...
		offset += STR_IntToStrEx(buffer + offset, -exponent, false, 10);
	}
...
STR_API void STR_PrintVA(STR_Builder* s, const char* fmt, va_list args) {
	for (const char* c = fmt; *c; c++) {
		if (*c == '%') {
			...
			case 'f': {
				char buffer[64];
				size_t size = STR_FloatToStrEx(buffer, va_arg(args, double), 0);
				STR_View value_str = { buffer, size };
			...
			print_int:;
				...
				char buffer[100];
				size_t size = STR_IntToStrEx(buffer, value, is_signed, radix);
				STR_View value_str = { buffer, size };
				STR_PrintV(s, value_str);
			} break;
			}
