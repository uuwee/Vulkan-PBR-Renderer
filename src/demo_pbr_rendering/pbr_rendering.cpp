...
#define ASSETS_LIST\
	...
	ASSET(SHADER_MAIN,                     "../src/demo_pbr_rendering/shader.glsl")\
	ASSET(SHADER_SKYBOX,                   "../src/demo_pbr_rendering/skybox_shader.glsl")\
	ASSET(SHADER_GEN_IRRADIANCE_MAP,       "../src/demo_pbr_rendering/gen_irradiance_map.glsl")\
	ASSET(SHADER_GEN_PREFILTERED_ENV_MAP,  "../src/demo_pbr_rendering/gen_prefiltered_env_map.glsl")\
	ASSET(SHADER_GEN_BRDF_INTEGRATION_MAP, "../src/demo_pbr_rendering/gen_brdf_integration_map.glsl")\
...
static GPU_String GPU_StringInit(const char* str) {
	return {(char*)str, (uint32_t)strlen(str)};
}

static bool ReadEntireFile(DS_Arena* arena, const char* file, char** out_data, uint32_t* out_size) {
...
static bool ReloadMesh(Mesh* result, DS_Arena* temp_arena, const char* filepath, uint32_t material_id) {
	...
	for (cgltf_size primitive_i = 0; primitive_i < mesh->primitives_count; primitive_i++) {
		...
		uint32_t first_index = (uint32_t)index_buffer.length;
		uint32_t first_index_value = (uint32_t)vertex_buffer.length;
		DS_ArrResizeUndef(&index_buffer, index_buffer.length + (int)indices->count);
	...
	*result = Mesh{};
	result->num_indices = (uint32_t)index_buffer.length;
	
	GPU_DestroyBuffer(result->vertex_buffer);
	result->vertex_buffer = GPU_MakeBuffer((uint32_t)vertex_buffer.length * sizeof(*vertex_buffer.data), GPU_BufferFlag_GPU, vertex_buffer.data);
...
static GPU_ComputePipeline* LoadComputeShader(DS_Arena* arena, AssetIndex asset, GPU_PipelineLayout* pipeline_layout, GPU_ShaderDesc* cs_desc) {
	for (;;) {
		...
		while (!ReadEntireFile(arena, ASSET_PATHS[asset], &shader_src.data, &shader_src.length)) {}

		cs_desc->glsl = shader_src;
		cs_desc->glsl_debug_filepath = GPU_StringInit(ASSET_PATHS[asset]);

		GPU_GLSLErrorArray errors = {0};
...
int main() {
	...
	for (;;) {
		...
		Camera_Update(&camera, &input_frame, 0.01f, 0.01f, 70.f, (float)window_w / (float)window_h, Z_NEAR, Z_FAR);
